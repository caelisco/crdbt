...
func usage(err string) {
	fmt.Println("crdbt: a command line utility to help work with Cockroach DB")
	if err != "" {
		fmt.Println("\nerror: ", err)
		fmt.Println()
	}
	...
	fmt.Println()
	txtformat("version", "output the version of crdbt and Cockroach DB")
	txtformat("update", "check to see if there are any updates available for Cockroach DB")
	txtformat("upgrade <version>", "upgrade Cockroach DB to the specified version")
	txtformat("upgrade latest", "upgrade Cockroach DB to the latest version based on the releases page")
	txtformat("list", "list all releases of Cockroach DB")
	txtformat("download <version>", "download the specified version of Cockroach DB")
	txtformat("download latest", "download the latest version of Cockroach DB based on the releases page")
	txtformat("extract <file>", "extract the contents from the specified .tgz file")
	fmt.Println("\nsystemd commands")
	txtformat("systemd status", "display the status of the Cockroach DB service")
	txtformat("systemd start", "start the Cockroach DB service")
	txtformat("systemd stop", "stop the Cockroach DB service")
	txtformat("systemd restart", "restart the Cockroach DB service")
	txtformat("systemd reload", "reload the Cockroach DB service")
	txtformat("systemd enable", "enable the Cockroach DB service to run at boot")
	txtformat("systemd disable", "disable the Cockroach DB service running at boot")
